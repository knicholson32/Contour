/*
  ...
  - You are about to drop the column `sim` on the `legs` table. All the data in the column will be lost.
  - Made the column `endTime_utc` on table `legs` required. This step will fail if there are existing NULL values in that column.
  - Made the column `startTime_utc` on table `legs` required. This step will fail if there are existing NULL values in that column.
