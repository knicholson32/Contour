...
export const load = async ({ parent, fetch, url }) => {
  ...
  const set = await settings.getMany('system.debug', 'system.lastSeenCommit', 'entry.tour.current', 'general.gravatar.hash', 'general.name', 'general.email', 'general.timezone', 'general.prefers_globe', 'tour.defaultStartApt', 'entry.dataVersion', 'general.cartoAPI', 'general.stadiaAPI');
