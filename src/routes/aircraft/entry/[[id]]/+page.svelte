<script lang="ts">
  ...
  let selectedReg: string | null = $state(null);
  let registrationExists = $state(false);
  ...
  let reg = $derived(page.url.searchParams.get('reg'));
</script>

<TwoColumn {ref} menuZone="scroll" formZone="scroll" bind:urlActiveParam bind:isMobileSize backText="Back">
  ...
  {#snippet form()}
    <div class="flex-shrink">
      ...
      <form id="form-create-or-mod" action="?/createOrModify&{page.url.search}" method="post" enctype="multipart/form-data" use:enhance={() => {
        ...
        <Section title="Aircraft Image" error={formData !== null && formData.ok === false && formData.action === '?/default' && formData.name === 'image' ? formData.message : null}>
