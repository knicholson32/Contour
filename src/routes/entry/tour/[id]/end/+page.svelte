  ...
  <div class="shrink">
    ...
    <form method="post" enctype="multipart/form-data" use:enhance={() => {
      ...
      <div class="inline-flex -mt-[2px] py-3 px-5 w-full flex-row gap-3 justify-end sticky bottom-0 z-10">
        <a href="/entry/tour/{page.params.id}" class="grow w-full text-center md:w-48 md:grow-0 touch-manipulation select-none transition-colors px-3 py-2 rounded-md text-sm font-semibold shadow-xs focus-visible:outline-solid focus-visible:outline-2 focus-visible:outline-offset-2 
        ...
        {/if}
        <Submit class="grow w-full md:w-48 md:grow-0" failed={form?.ok === false && (form.action === '?/default' || form?.action === '*')} {submitting} theme={{primary: 'white'}} actionText="End Tour" actionTextInProgress="Ending Tour" />
      </div>
    </form>
  </div>
