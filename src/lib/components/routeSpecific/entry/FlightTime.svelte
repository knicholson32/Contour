<script lang="ts">
   ...
   */
  let autoFillFunc = () => {
    if (autoFill !== null) input.value = autoFill.toFixed(1);
    input.blur();
  }
...
<Frame bind:title focus={focus} bind:disabled>
  ...
  <input tabindex="0" maxlength="4" bind:this={input} disabled={disabled} on:change={_update} on:input={_updateContinuous} pattern="[0-9]*" type="number" bind:value placeholder="0.0" name={name}
