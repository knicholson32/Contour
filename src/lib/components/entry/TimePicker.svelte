        ...
        <div bind:this={dialog} use:EscapeOrClickOutside={{ callback: closeDialog }} class="absolute right-0 top-11 xs:top-10 xs:right-3 z-10 p-3 w-full xs:w-96 origin-top-right xs:rounded-md bg-white shadow-lg ring-1 ring-gray-300 ring-inset focus:outline-none flex flex-col gap-3" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
          ...
          <hr class="mb-1"/>
            <div class="flex flex-row gap-3 items-center relative">
              ...
              {#if tz !== 'UTC'}
                <button disabled={disabled} on:click={convertToUTC} type="button" class="touch-manipulation select-none relative whitespace-nowrap text-xs transition-colors flex justify-center items-center px-3 py-2 rounded-md font-semibold shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed ring-1 ring-gray-300 text-gray-800 betterhover:hover:bg-gray-100 betterhover:hover:text-gray-900 disabled:bg-gray-50 disabled:text-gray-500 disabled:ring-gray-200 bg-white  focus-visible:outline-grey-500">Convert to UTC</button>
              {/if}
            </div>
