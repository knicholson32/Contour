  ...
  <div class="w-full relative my-2 flex flex-col items-center px-3 py-1 gap-1 {disabled ? 'cursor-not-allowed bg-gray-50 text-gray-500' : ''}">
    <div class="col-span-full w-full">
      <div role="presentation" class=" group relative mx-auto flex justify-center rounded-lg max-w-sm" on:click={clickedArea} on:mouseleave={mouseLeave} on:dragenter={startDrag} on:dragleave={endDrag} on:dragover={(e) => e.preventDefault()} on:drop={handleDrop}>
        ...
        <div class="bg-gray-100 dark:bg-transparent {((_initialImageId === null && previewString === '') || dragging || currentState === ImageUploadState.DELETE || forceShowMenu) && !forceImageDisplay ? 'opacity-100' : 'opacity-0'} {forceImageDisplay ? '' : 'betterhover:group-hover:opacity-100'} transition-opacity text-center absolute inset-0 flex flex-col items-center justify-center rounded-lg border-dashed border-gray-300 dark:border-zinc-700 {dragging ? 'border-sky-400 border-2' : 'border'}">
