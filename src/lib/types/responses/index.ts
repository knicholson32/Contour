 ...
 */
export const _404 = () => {
	return json({ status: 404, ok: false, message: 'Not found' } satisfies API.Error, {
		status: 404
	});
};
 ...
 */
export const _400 = (config: {
	message?: string;
	missingPaths?: string[];
	missingBodyParams?: string[];
	missingURLParams?: string[];
}) => {
	if (config.message !== undefined)
		return json({ status: 400, ok: false, message: config.message } satisfies API.Error, {
			status: 400
		});
	if (config.missingPaths !== undefined)
		return json(
			{
				status: 400,
				ok: false,
				message: 'Missing inputs: ' + config.missingPaths.join(', ')
			} satisfies API.Error,
			{ status: 400 }
		);
	if (config.missingBodyParams !== undefined)
		return json(
			{
				status: 400,
				ok: false,
				message: 'Missing JSON body params: ' + config.missingBodyParams.join(', ')
			} satisfies API.Error,
			{ status: 400 }
		);
	if (config.missingURLParams !== undefined)
		return json(
			{
				status: 400,
				ok: false,
				message: 'Missing URL search params: ' + config.missingURLParams.join(', ')
			} satisfies API.Error,
			{ status: 400 }
		);
	else
		return json({ status: 400, ok: false, message: 'Missing inputs' } satisfies API.Error, {
			status: 400
		});
};
...
export const success = (opts?: { status?: number; bool?: boolean }) => {
	if (opts !== undefined) {
		...
		if (opts.bool !== undefined)
			return json({ status, ok: true, type: 'boolean', value: opts.bool } satisfies API.Boolean, {
				status
			});
	}
		...
		return json(
			{
				status: 500,
				ok: false,
				message: `Server error. Context="${context}" | ${error}`
			} satisfies API.Error,
			{ status: 500 }
		);
	else
		return json(
			{ status: 500, ok: false, message: `Server error | ${error}` } satisfies API.Error,
			{ status: 500 }
		);
};
